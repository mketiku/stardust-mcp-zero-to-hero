...
class StardustOperationsServer(
    ...
    fun searchPersonnel(
            @McpToolParam(description = "Name or PID. Example: 'Cmdr. Orion' or 'PID-101'")
            query: String,
    ...
    fun getBayStatus(
            @McpToolParam(description = "Numeric deck identifier. Example: '10' or '15'")
            deck: String? = null,
    ...
    fun assignBay(
            @McpToolParam(description = "The specific bay ID. Example: 'BAY-ALPHA-1'")
            bayId: String,
            @McpToolParam(description = "Personnel ID. Example: 'PID-202'") pid: String,
    ): String = mockSystem.assignBay(bayId, pid)

    @McpTool(description = "Record a duty shift/rotation for personnel.")
    fun recordDutyShift(
            @McpToolParam(description = "Personnel ID. Example: 'PID-101'") pid: String,
            @McpToolParam(description = "Start cycle in SOL format. Example: 'SOL-2026.04.12'")
            startCycle: String,
            @McpToolParam(description = "End cycle in SOL format. Example: 'SOL-2026.04.13'")
            endCycle: String,
            @McpToolParam(description = "Shift category. Example: 'Maintenance' or 'Command'")
            type: String,
