...
class StardustOperationsServer(
    ...
    fun searchPersonnel(query: String): List<Personnel> = mockSystem.searchPersonnel(query)

    @McpTool(description = "Get status of docking bays, optionally filtered by deck.")
    fun getBayStatus(deck: String? = null): List<BayInfo> = mockSystem.getBays(deck)
    ...
    suspend fun getPersonnelDeployment(pid: String): String =
        coroutineScope {
